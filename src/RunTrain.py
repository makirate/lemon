from pandas import read_csv
from PIL import Image
import cv2
import numpy as np
from sklearn.model_selection import train_test_split
import tensorflow as tf
import keras
import os #?
import glob
from efficientnet.keras import center_crop_and_resize, preprocess_input

lemon_place = os.path.dirname(__file__).rstrip('/src')
test_images_place = lemon_place + "/datas/transform_lemons/test_images"
train_images_place = lemon_place + "/datas/transform_lemons/train_images"

"""
DIR = train_images_place
file_sum = sum(os.path.isfile(os.path.join(DIR, name)) for name in os.listdir(DIR))
"""

def get_labels(place):
    df = read_csv(place,engine='python')
    df = df["class_num"]
    labels = df.values
    return label_list

#出力値はそのまま使える
def get_imgs(place,num_file):
    img_list = []
    for i in range(num_file):
        file_name = '/' + str(i).zfill(4)
        img = np.array(cv2.imread(place + file_name + '.jpg'))
        image_size = 600 #B7なので。本来ならclassを使い定義すべきか？
        img = center_crop_and_resize(img, image_size=image_size)
        img = preprocess_input(img)
        img = np.expand_dims(img, 0)
        img_list.append(img)
        print(np.shape(img))
    return np.array(img_list)

def run_train():
    img_list = get_imgs(place,num_file)
    label_list = get_labels(place)
    train_img,train_label,test_img,test_label = train_test_split(img_list,label_list)

    base_model = tf.keras.applications.EfficientNetB7(input_shape=(600,600,3), weights='imagenet', include_top=False)
    x = keras.layers.GlobalAveragePooling2D()(base_model.output)
    output = keras.layers.Dense(4, activation='softmax')(x)
    model = keras.models.Model(inputs=[base_model.input], outputs=[output]) 
    model.compile(optimizer='SGD', loss='categorical_crossentropy', metrics=['accuracy'])
    model.fit(x=img_list,y=label_list,epochs=20,validation_split=0.2)
    model.save(lemon_place+'/fine_model.h5')


if __name__ == "__main__":
    get_imgs(train_images_place,3)
    get_labels(lemon_place + '/datas/train_images.csv')
    model.fit(X_train_processed, y_train_categorical)
